...
const BASE_URL = "https://trackexpence-backend.onrender.com/api";
...
const Profile = ({ user: propUser, onUpdateProfile, onLogout }) => {
  ...
  // keep local state synced if prop user is passed
  useEffect(() => {
    if (propUser) {
      const normalized = normalizeUser(propUser);
      setUser(normalized);
      setTempUser(normalized);
    }
  }, [propUser, normalizeUser]);
  ...
  }, [handleApiRequest, propUser, normalizeUser]);
