...
const API_BASE = 'https://trackexpence-backend.onrender.com/api';
...
const Dashboard = () => {
  ...
  console.log("overviewMeta", overviewMeta);
  ...
  const financialOverviewData = useMemo(() => {
    ...
    return Object.keys(categories)
      .map((category) => ({
        name: category,
        value: Math.round(Number(categories[category]) || 0),
      }))
      .filter((item) => item.value > 0);
        ...
        <FinancialCard
          ...
          additionalContent={
            ...
            >
              ...
              <span>
                {Math.abs(expenseChange)}% {expenseChange >= 0 ? 'increase' : 'decrease'} from{' '}
        ...
        <div className={dashboardStyles.pieChartHeight}>
          {financialOverviewData.length > 0 ? (
            ...
            <div className={dashboardStyles.emptyState}>
              ...
              <p className={dashboardStyles.emptyText}>No expense distribution data available</p>
            </div>
          )}
        </div>
      </div>
        ...
        <div className={dashboardStyles.listContainer}>
          ...
          <div className={dashboardStyles.transactionList}>
            {displayedIncome.map((transaction) => {
              const IconComponent =
                INCOME_CATEGORY_ICONS[transaction.category] || INCOME_CATEGORY_ICONS.Other;

              return (
                <div key={transaction.id} className={dashboardStyles.incomeTransactionItem}>
                  <div className={dashboardStyles.transactionContent}>
                    {/* <div className={dashboardStyles.incomeIconContainer}>{IconComponent}</div> */}
  ...
  {React.cloneElement(IconComponent, {
    ...
    children: "₹",
                    ...
                    <div className="min-w-0 flex-1">
                      <p className={`${dashboardStyles.transactionDescription} truncate`}>
                  ...
                  <div className={`${dashboardStyles.transactionAmount} text-right shrink-0`}>
            ...
            {incomeListForDisplay.length === 0 && (
              <div className={dashboardStyles.emptyState}>
                <div className={dashboardStyles.emptyIconContainer('bg-green-50')}>
                  <span className="w-8 h-8 flex items-center justify-center text-green-400 font-bold text-2xl">
                    ₹
                  </span>
                </div>
        ...
        <div className={dashboardStyles.listContainer}>
          ...
          <div className={dashboardStyles.transactionList}>
            {displayedExpense.map((transaction) => {
              const IconComponent =
                EXPENSE_CATEGORY_ICONS[transaction.category] || EXPENSE_CATEGORY_ICONS.Other;

              return (
                <div key={transaction.id} className={dashboardStyles.expenseTransactionItem}>
                  <div className={dashboardStyles.transactionContent}>
                    <div className={dashboardStyles.expenseIconContainer}>{IconComponent}</div>
